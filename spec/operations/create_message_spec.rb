...
RSpec.describe CreateMessage do
  ...
  let(:ai_response)  { AiResponse.new(content: "Respuesta de prueba", model: "llama3.2:3b", provider: "ollama") }
  let(:mock_client)  { instance_double(Ai::OllamaClient) }

  before do
    allow(Ai::Dispatcher).to receive(:for).with("ollama").and_return(mock_client)
